...
import clear.ui.panel.CoopPlayerRoomPanel.FriendsDialog;
import clear.ui.panel.CoopPlayerRoomPanel.recThread;
//import clear.ui.panel.ReadyPanel.FriendsDialog;
import enupackage.ButtonType;
import enupackage.PanelType;


public class CoopHosterRoomPanel extends JPanel implements MouseListener{
	...
	public CoopHosterRoomPanel(String roomName ,String roomMaster ,String name[],int head[],String level[],String gold[],boolean ready[],int state[],JFrame boot){
		bootFrame = boot;
		this.name=name;
...
System.out.println(message);
						...
						if(temp.length == 2){
							if(temp[1].equals("已添加您为好友")){
								MyTipsDialog mtd = new MyTipsDialog(temp[0]+temp[1]);
							...
							if(temp[0].equals("系统消息:")){
								if(temp[2].equals("邀请您加入他/她的")){
									EnterRoomDialog erd = new EnterRoomDialog(message);
						}else if(temp[2].equals("拒绝加入您的房间")||
									temp[2].equals("已经离开您的房间")){

									MyTipsDialog mtd =new MyTipsDialog(message);
...
System.out.println(message);
									...
									state[count-1] = 2 ;
									head[count] = Integer.parseInt(temp[3]) ;
									level[count] =(temp[4]) ;
									gold[count] =(temp[5]) ;
									repaint();
								}else if(temp[2].equals("申请加入您的")){
									EnterRoomDialog2 ergd = new EnterRoomDialog2(message);
...
System.out.println("在线好友:"+s);
										lists.add(s);
									}
									if(lists.get(0).equals("空")){
										MyTipsDialog mtd = new MyTipsDialog("没有好友，快去添加");
									}else{
								...
								if(temp[1].equals("您已被")){
									MyTipsDialog mtd = new MyTipsDialog(temp[1]+temp[2]+temp[3]);
								}
							}
						}
					}
	...
	private void change1(String msg ,int type) throws IOException{
	...
	private void change2(String msg,int type) throws IOException{
		String[] temp = msg.split(" ");
		if(type == 0){
	...
	private void change3(String msg,int type) throws IOException{
		String[] temp = msg.split(" ");
		if(type == 0){
			int count = 0 ;
			for(String n : name){
			...
			state[count-1] = 2 ;
			head[count] = Integer.parseInt(temp[5]) ;
			level[count] =(temp[6]) ;
			gold[count] =(temp[7]) ;
			repaint();
	...
	protected void paintComponent(Graphics g){
		...
		for(int i=0;i<3;i++){
//System.out.println("here state : "+state[i]);
			switch(state[i]){
			case 0:    //加号状态
			...
			case 2:
				...
				/*if(ready[i]){
					g2d.drawImage(readyImage,loc[i+1].x, loc[i+1].y, 62,64,this);
				}*/
				break;
			}
			for(int j = 0 ;j<4 ; j++){
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(source==readyLabel){
			...
			ToolPanel2 tp = new ToolPanel2(roomName,
					roomMaster,Player.getGold(),"src/images/toolpanel/toolpanel2.png",0+"" ,0,bootFrame);
			tp.setSize(927, 661);
			tp.setLocation(0, 0);
			add(tp);
			repaint();
			...
			if(state[0]==0){
				output.println("数据 显示在线好友请求 "+Player.getUserName()+" "+0);
				output.flush();
			}else if(state[0]==1){
				return;
			}else if(state[0]==2){
			...
			System.out.println("邀请好友游戏");
			if(state[1]==0){
				output.println("数据 显示在线好友请求 "+Player.getUserName()+" "+1);
				output.flush();
				repaint();
			}else if(state[1]==1){
				return;
			...
			System.out.println("邀请好友游戏");
			if(state[2]==0){
				output.println("数据 显示在线好友请求 "+Player.getUserName()+" "+2);
				output.flush();
				System.out.println("邀请好友游戏");
				repaint();
			}else if(state[2]==1){
...
System.out.println("Here1");
					state[tag] = 1 ;repaint();
					output.println("操作 邀请加入房间 "+roomName + " "+nameList.get(pages*3)+" "+Player.getUserName());
					output.flush();
				...
				if (((pages + 1) == panelnum) && (panelnum * 3 - infonum) >= 2) {
					state[tag] = 1 ;repaint();
					output.println("操作 邀请加入房间 "+roomName + " "+nameList.get(pages*3+1)+" "+Player.getUserName());
					output.flush();
				...
				if (((pages + 1) == panelnum) && (panelnum * 3 - infonum) >= 1) {
					state[tag] = 1 ;
					output.println("操作 邀请加入房间 "+roomName + " "+nameList.get(pages*3+2)+" "+Player.getUserName());
					output.flush();
					repaint();
				} else {
					state[tag] = 1 ;
	...
	private class MyTipsDialog extends JFrame implements MouseListener{
		JPanel backPanel;
		JLabel close;
		JLabel msg;
		JLabel yes;
		JLabel cancle;
		public MyTipsDialog(String msgs){
	...
	private class EnterRoomDialog2 extends JFrame implements MouseListener{
		JPanel backPanel;
		JLabel close;
		JLabel msg;
		JLabel yes;
		...
		JLabel cancle;
		String tempMsg;
		public EnterRoomDialog2(String msg0){
			tempMsg = msg0;
			String[] temp = msg0.split(" ");
			backPanel = new DialogPanel("src/images/Dialog/OtherDialog/ask.png");
			backPanel.setLayout(null);
