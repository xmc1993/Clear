...
import clear.ui.network.CreateRoom2Client;
import clear.ui.network.CreateRoomClient;
import clear.ui.panel.*;

public class FightGameHall extends JPanel implements MouseListener {
	...
	// private Hashtable<String,String> roomInfo3 = new
	...
	public FightGameHall(ArrayList<String> roomData, int tag, JFrame boot) {
		bootFrame = boot;
		this.tag = tag;
		if (roomData.get(0).equals("")
			...
			for (String s : roomData) {
				...
				roomInfo.put(s.split(" ")[0], s.split(" ")[2]);// 房主名
				roomInfo2.put(s.split(" ")[0], s.split(" ")[1]);// 房间名
		...
		if (source == up) {
			...
			if (existed) {
				...
				if (Player.getUserName().equals(
					...
					FightHosterRoomPanel rp = new FightHosterRoomPanel(
							roomInfo.get(field.getText().trim()),
							roomInfo2.get(field.getText().trim()), name, head,
							lvl, gold, ready, state, name2, head2, lvl2, gold2,bootFrame);
					removeAll();
					rp.setBounds(0, 0, 927, 661);
					add(rp);
					repaint();
				} else {
					FightPlayerRoomPanel rp = new FightPlayerRoomPanel(
							roomInfo.get(field.getText().trim()),
							roomInfo2.get(field.getText().trim()), name, head,
							lvl, gold, ready, state, name2, head2, lvl2, gold2,bootFrame);
					removeAll();
			...
			if (Player.getUserName().equals(getRoom.get(addPanel[0]))) {
				FightHosterRoomPanel rp = new FightHosterRoomPanel(
						...
						name, head, lvl, gold, ready, state, name2, head2,
						lvl2, gold2,bootFrame);
				removeAll();
				rp.setBounds(0, 0, 927, 661);
				add(rp);
				repaint();
...
System.out.println("room:" + room);
		name[0] = room.split(" ")[2];
		int i = 1;
		for (String s : room.split(" ")[5].split("%")) {
			if (s.equals("空")) {
		...
		i = 0;
		for (String s : room.split(" ")[6].split("%")) {
			if (s.equals("空")) {
		...
		if(name2[0].equals("")&&name2[1].equals("")){
			
		}else{
			String[] info2 = data.show(name2).split(";");
			int j = 0;
			for (String s : info) {
			...
			for (String s : info2) {
				System.out.println("info2:" + s);
				String[] temp = s.split("_");
				if(temp[0].equals("")){
					
				}else{
					head2[j] = Integer.parseInt(temp[2]);
					lvl2[j] = temp[3];
					gold2[j] = temp[4];
				}
	...
	public void createRoom(String name, int limit) {
		CreateRoom2Client client = new CreateRoom2Client();

		String result = client
				.connect(name, Player.getUserName(), limit, limit);
		if (result.equals("Success")) {
			...
			FightHosterRoomPanel rp = new FightHosterRoomPanel(name,
					Player.getUserName(), names, head, lvl, gold, ready, state,
					name2, head2, lvl2, gold2,bootFrame);
			...
			TipsDialog td = new TipsDialog("房间已创建，不能再建");
		}
	}
